...
# negative binomial ancestor
p_nb=.4
size_nb=4
sa=rnbinom(1000,size=size_nb,prob=p_nb)+size_nb
...
var(sa)
sigma2_A = (1-p_nb) * size_nb  /  p_nb^2
sigma2_A
...
for (jj in 1:4)
{
  ...
  for (b in (1:B))
  {
      ...
      for (j in (1:J) )
    {
      ...
      repeat
      {
      z0=rnbinom(1,size=size_nb,prob=p_nb)+size_nb
      if (z0>0){break}
      }
